...
def send_from_custom_transaction(
        ...
        txin_scriptPubKey, txout_scriptPubKey,tx_type,first_private_key, second_private_key, final_private_key,network):
    txout = create_txout(amount_to_send, txout_scriptPubKey)
    txin = create_txin(txid_to_spend, utxo_index)

    if(tx_type==2):
        txin_scriptSig=[OP_0,create_OP_CHECKSIG_signature(txin, txout, txin_scriptPubKey, first_private_key),
                        create_OP_CHECKSIG_signature(txin, txout, txin_scriptPubKey, second_private_key)
                        ]
    else:
        txin_scriptSig = [OP_0,create_OP_CHECKSIG_signature(txin, txout, txin_scriptPubKey, first_private_key),
                          create_OP_CHECKSIG_signature(txin, txout, txin_scriptPubKey, second_private_key),
                          OP_0, create_OP_CHECKSIG_signature(txin, txout, txin_scriptPubKey, final_private_key)
                          ]
